...
class MultipleNode(Node):
    def __init__(self, code, description, parent, varcontrol, formatvarcontrol, values, seuils,  valueType = 'none', valueFormat = 'none'):
        ...
        self.seuils = seuils
        self.values = values
        self.formatcontrol = formatvarcontrol
        ...
        if nb_values>1 :
            child = SubElement(parent, 
                                ...
                                attrib = {'code': self.code,
                                          ...
                                          'formatcontrol': self.formatcontrol})
        ...
        for element in nodeList:
            if element.nodeType is not element.TEXT_NODE and element.nodeType is not element.COMMENT_NODE:
                if element.tagName == "BAREME":
                    ...
                    varcontrol, formatcontrol = self.handleCodes(element.getElementsByTagName("VARCONTROL")[0], date)
                    if not varcontrol is None :
                        values, seuils = multiple_lists(element, date, formatcontrol)
                        node = MultipleNode(code, desc, parent, varcontrol, formatcontrol, values, seuils, valueType, valueFormat)
    ...
    def handleCodes(self, element, date):
        for code in element.getElementsByTagName("CODE"):
            try:
                ...
                if deb <= date <= fin:
                    return code.getAttribute('code'), code.getAttribute('format')
            except Exception, e:
                code = element.getAttribute('code')
                raise Exception("Problem error when dealing with %s : \n %s" %(code,e))
        return None, None
