...
def multiple_lists(element, date, formatcontrol):
    seuils = []
    values = []
    for val in element.getElementsByTagName("VALUE"):
        try:
            ...
            if deb <= date <= fin:
                ...
                if formatcontrol == 'date' :
                    seuil =  datetime.strptime(val.getAttribute('valeurcontrol'),"%Y-%m-%d").date()
                if formatcontrol == 'integer' : 
                    seuil =  int(val.getAttribute('valeurcontrol'))
...
def TranchesAttr(node, V, S):
        nb_tranches = len(S)
        if node.formatcontrol == 'date' :
            S += [datetime.strptime('2100-01-01',"%Y-%m-%d").date()]
        if node.formatcontrol == 'integer' :
            S+= [900]
